...
auth_token:string;
  ...
  resetPassword(data:any){caches
    debugger;
    let Authorizationt=  'token'+'\xa0'+window.localStorage.getItem('token');
    const httpOptions = {
    ...
    httpOptions.headers =httpOptions.headers.set('Authorization', Authorizationt);
    //let reqheader = new HttpHeaders({'Content-Type': 'application/x-www-form-urlencoded'}).set('Authorization',this.Authorization);
    return this.http.post(this.APIUrl + '/AdminAngular/api/change-password/',data,httpOptions);
  }
